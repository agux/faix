...
class WaveNetModel(object):
    ...
    def _create_dilation_layer(self, input_batch, layer_index, dilation,
        ...
        if self.histograms:
            ...
            tf.compat.v1.summary.histogram(layer + '_filter', weights_filter)
            tf.compat.v1.summary.histogram(layer + '_gate', weights_gate)
            tf.compat.v1.summary.histogram(layer + '_dense', weights_dense)
            tf.compat.v1.summary.histogram(layer + '_skip', weights_skip)
            if self.use_biases:
                tf.compat.v1.summary.histogram(layer + '_biases_filter', filter_bias)
                tf.compat.v1.summary.histogram(layer + '_biases_gate', gate_bias)
                tf.compat.v1.summary.histogram(layer + '_biases_dense', dense_bias)
                tf.compat.v1.summary.histogram(layer + '_biases_skip', skip_bias)
    ...
    def _create_network(self, input_batch, global_condition_batch):
        ...
        with tf.compat.v1.name_scope('postprocessing'):
            ...
            if self.histograms:
                tf.compat.v1.summary.histogram('postprocess1_weights', w1)
                tf.compat.v1.summary.histogram('postprocess2_weights', w2)
                if self.use_biases:
                    tf.compat.v1.summary.histogram('postprocess1_biases', b1)
                    tf.compat.v1.summary.histogram('postprocess2_biases', b2)
