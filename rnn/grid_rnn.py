...
from tensorflow import logging, nn
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__):  # GridRNNCell
            ...
            if self._output_is_tuple:
                ...
                if output_tensors:
                    output = tf.concat(output_tensors, 1)
                else:
                    output = tf.zeros([0, 0], dtype)
            ...
            if self._state_is_tuple:
                ...
                if state_tensors:
                    states = tf.concat(state_tensors, 1)
                else:
                    states = tf.zeros([0, 0], dtype)
    ...
    def _extract_states(self, state):
        ...
        if self._state_is_tuple:
            ...
            for recurrent_dim, start_idx in zip(conf.recurrents,
                ...
                if cell_output_size > 0:
                    c_prev[recurrent_dim] = tf.slice(state, [0, start_idx],
                                                     [-1, conf.num_units])
                    m_prev[recurrent_dim] = tf.slice(
                        state, [0, start_idx + conf.num_units], [-1, cell_output_size])
                else:
                    m_prev[recurrent_dim] = tf.slice(state, [0, start_idx],
                                                     [-1, conf.num_units])
            ...
            for i, j in enumerate(conf.inputs):
                ...
                if with_c:
                    input_project_c = tf.get_variable(
                        ...
                        dtype=inputs.dtype)
                    c_prev[j] = tf.matmul(
                        input_splits[i], input_project_c)
        ...
        if isinstance(state_sizes, tuple):
            ...
            for e in state_sizes:
                if isinstance(e, tf.nn.rnn_cell.LSTMStateTuple):
                    ...
                    break
            if hastuple:
                return sum([sum(s) for s in state_sizes])
            else:
...
def _propagate(dim_indices, conf, cells, c_prev, m_prev, new_output, new_state,
    ...
    if conf.num_dims > 1:
        ...
        cell_inputs = tf.concat(ls_cell_inputs, 1)
    else:
        cell_inputs = tf.zeros([m_prev[0].get_shape().as_list()[0], 0],
                               m_prev[0].dtype)
